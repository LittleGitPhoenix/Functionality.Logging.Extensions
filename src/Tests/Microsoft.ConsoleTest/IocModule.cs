...
class IocModule : Autofac.Module
{
	...
	private static void RegisterLoggers(ContainerBuilder builder)
	{
		...
		builder
			...
			.Register(context => context.Resolve<Func<ILogger>>().Invoke()) 
		...
		builder
			...
			.Register(context => context.Resolve<Func<string, ILogger>>().Invoke("MyLogger"))
